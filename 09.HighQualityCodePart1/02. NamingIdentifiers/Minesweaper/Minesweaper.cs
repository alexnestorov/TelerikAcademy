...
namespace Minesweaper
{
    public class Minesweaper
    {
        public static void Main(string[] args)
        {
            ...
            int movesCounter = 0;
            List<GameScore> champions = new List<GameScore>(6);
            ...
            do
            {
                ...
                switch (command)
                {
                    ...
                    case "turn":
                        if (mines[row, col] != '*')
                        {
                            if (mines[row, col] == '-')
                            {
                                GenerateNextMove(gameField, mines, row, col);
                                movesCounter++;
                            }

                            if (movesCounter == MaximumEmptyCells)
                            {
                                isWin = true;
                            }
                            else
                ...
                if (isExplode)
                {
                    ...
                    GameScore score = new GameScore(nickname, movesCounter);
                    ...
                    champions.Sort((GameScore score1, GameScore score2) => score2.UserName.CompareTo(score1.UserName));
                    champions.Sort((GameScore score1, GameScore score2) => score2.UserPoints.CompareTo(score1.UserPoints));
                    GetScoreBoard(champions);
                ...
                if (isWin)
                {
                    ...
                    GameScore score = new GameScore(name, movesCounter);
                    champions.Add(score);
                    GetScoreBoard(champions);
                    gameField = CreateGameField();
                    mines = GenerateMines();
        ...
        private static void GetScoreBoard(List<GameScore> scores)
        {
            Console.WriteLine("\nPOINTS:");
            if (scores.Count > 0)
            {
        ...
        private static void GenerateNextMove(char[,] field, char[,] minesField, int currentRow, int currentCol)
        {
